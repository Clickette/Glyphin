...
module.exports = {
    ...
    async execute(message, args) {
        ...
        try {
            ...
            let totalCases;
            if (user) {
                const result = await db.all('SELECT * FROM logs WHERE server_id = ? AND user_id = ? ORDER BY timestamp DESC', [message.guild.id, user.id]);
                totalCases = result.data.length;
                cases = result.data.slice(offset, offset + limit);
            } else {
                const result = await db.all('SELECT * FROM logs WHERE server_id = ? ORDER BY timestamp DESC', [message.guild.id]);
                totalCases = result.data.length;
                cases = result.data.slice(offset, offset + limit);
            }
            ...
            if (!Array.isArray(cases) || cases.length === 0) {
                ...
                await message.channel.send({ embeds: [noCasesEmbed.toJSON()] });
                return;
            }
            ...
            const msg = await message.channel.send({ embeds, components: [buttonRow] });
            ...
            collector.on('collect', async (i) => {
                ...
                const offset = (page - 1) * limit;
                if (user) {
                    const result = await db.all('SELECT * FROM logs WHERE server_id = ? AND user_id = ? ORDER BY timestamp DESC LIMIT ? OFFSET ?', [message.guild.id, user.id, limit, offset]);
                    cases = result.data;
                } else {
                    const result = await db.all('SELECT * FROM logs WHERE server_id = ? ORDER BY timestamp DESC LIMIT ? OFFSET ?', [message.guild.id, limit, offset]);
                    cases = result.data;
                }
            ...
            await message.channel.send({ embeds: [errEmbed.toJSON()] });
        }
    }
};
